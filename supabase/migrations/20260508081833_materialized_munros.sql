...
-- Create as a materialized view instead
CREATE MATERIALIZED VIEW vu_munros AS
SELECT 
  m.*,
...
FROM munros m
LEFT JOIN reviews r        ON m.id = r.munro_id
LEFT JOIN munro_completions mc ON m.id = mc.munro_id
GROUP BY m.id;